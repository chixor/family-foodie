  ...
  */
  getRecipesForRandomisation () {
    ...
    this.limitedList = this.savedLimitedList = this.state.recipes.filter((r, x) => {
      return (discludeList.indexOf(r.id) === -1 && (r.season === null || r.season === thisSeason))
    })
  }
    ...
    return (
      <div className='row'>
        <div className='col-md-12'>
          <button className='btn btn-default' onClick={() => this.addWeek(this.state.nextWeek.getDateWeek(), this.state.nextWeek.getFullYear())}>
              ...
              <div key={`week-${w.week}-${w.year}`}>
                <h2>
                  Week {w.week}, {w.year}
                  {
                    w.unsaved
                    ? <span>
                      &nbsp<button className='btn btn-sm btn-default' onClick={() => this.randomizeAll(i)}><span className='glyphicon glyphicon-refresh' /> Randomize</button>
                      &nbsp<button className='btn btn-sm btn-success' onClick={() => this.save(i)}><span className='glyphicon glyphicon-ok' /> Save</button>
                      &nbsp<button className='btn btn-sm btn-danger' onClick={() => this.delete(i)}><span className='glyphicon glyphicon-remove' /> Delete</button>
                      &nbsp<button className='btn btn-sm btn-default' onClick={() => this.cancel(i)}><span className='glyphicon glyphicon-remove' /> Cancel</button>
                    </span>
                    : (w.cost !== null && typeof w.cost !== 'undefined')
                      ? <span>&nbsp&nbsp|&nbsp&nbsp${w.cost.toFixed(2)}</span>
                      : null
                  }
                  {
                    !w.unsaved
                    ? <span>
                      &nbsp<button className='btn btn-sm btn-default' onClick={() => this.edit(i)}><span className='glyphicon glyphicon-pencil' /> Edit</button>
                    </span>
                    : null
                }
                </h2>
