...
export default class Planner extends Component {
    ...
    newWeekObject = (week, year) => {
      return {
        ...
        unsaved: true,
        firstDay: new MenuDate(year,0,(1 + (parseInt(week) - 1) * 7)).toFirstDayOfTheWeek(),
        lastDay: new MenuDate(year,0,(1 + (parseInt(week) - 1) * 7)).toLastDayOfTheWeek()
      }
    }
    ...
    getWeeks = () => {
        api.getWeeks().then((weeks) => {
            ...
            const nextWeek = new MenuDate(weeks[0].year,0,daysToWeek).nextDateWeek()
            weeks.forEach((w) => {
              w.firstDay = new MenuDate(w.year,0,(1 + (parseInt(w.week) - 1) * 7)).toFirstDayOfTheWeek()
              w.lastDay = new MenuDate(w.year,0,(1 + (parseInt(w.week) - 1) * 7)).toLastDayOfTheWeek()
            })
    ...
    delete = (index) => {
      api.deleteWeek(this.state.weeks[index]).then(() => {
        ...
        const nextWeek = new MenuDate(weeks[0].year, 0, (1 + (weeks[0].week - 1) * 7)).nextDateWeek()
        this.setState({ weeks, nextWeek })
      })
    }
                ...
                <div key={`week-${w.week}-${w.year}`}>
                  <h2 className="shopping-week">
                    ...
                    }
                    {
                      !w.unsaved ?
                        <span>
                          &nbsp;<button title="Edit" className="btn btn-sm btn-default" onClick={() => this.edit(i)}><span className="glyphicon glyphicon-pencil"></span></button>
                          &nbsp;<a title="Shopping List" href={`/shopping/${w.year}/${w.week}`} className="btn btn-sm btn-default"><span className="glyphicon glyphicon-list"></span></a>
                          { w.cost ? <span className="pricetag"><sup>$</sup>{w.cost.toFixed(2)}</span> : null }
                        </span>
                        :null
                    }
                  </h2>
                                  ...
                                  <ul className="search-results">
                                    {
                                      this.state.searchResults.map((r,x) =>
                                        <li key={`recipe-search-results-${r.id}`} onClick={() => this.addSearchOne(i,r)}><img alt="thumbnail" src={`/assets/resources/${r.front}.jpg`}/> <span>{r.name}</span></li>
                                      )
                                    }
                                  </ul>:
                                  null
