...
export default class Planner extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      recipes: [],
      thisWeek: new Date(),
      nextWeek: nextDateWeek(new Date()),
      searchResults: []
    };
  ...
  getWeeks() {
    api.getWeeks().then((weeks) => {
      this.setState(prevState => {
        if(weeks.length === 0 || (weeks.length > 0 && (parseInt(weeks[0].week,10) < getDateWeek(this.state.thisWeek) || parseInt(weeks[0].year,10) < this.state.thisWeek.getFullYear()))) {
          weeks.unshift({week: getDateWeek(this.state.thisWeek), year: this.state.thisWeek.getFullYear(), recipes: this.chooseRecipes(3), unsaved: true});
        }
        let daysToWeek = (1 + (weeks[0].week - 1) * 7);
        prevState.nextWeek = nextDateWeek(new Date(weeks[0].year,0,daysToWeek));
  ...
  addWeek(week,year) {
    this.setState(prevState => {
      ...
      prevState.nextWeek = nextDateWeek(prevState.nextWeek);
      return prevState;
    });
  }
  ...
  delete(index) {
    api.deleteWeek(this.state.weeks[index]).then(() => {
      this.setState(prevState => {
        ...
        prevState.nextWeek = nextDateWeek(new Date(prevState.weeks[0].year,0,daysToWeek));
        return prevState;
      });
    });
  }
    ...
    return (
      <div className="row">
        <div className="col-md-12">
          <button className="btn btn-default" onClick={() => this.addWeek(getDateWeek(this.state.nextWeek),this.state.nextWeek.getFullYear())}><span className="glyphicon glyphicon-plus"></span> Week {getDateWeek(this.state.nextWeek)}</button>
          <br/><br/>
          {
