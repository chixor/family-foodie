...
export default class Recipe extends React.Component {
  render () {
    ...
    if (typeof this.props.delete !== 'undefined') {
      deleteBtn = <div title='remove' className='corner-triangle-right' onClick={() => this.props.delete(this.props.windex, this.props.index)}><span className='glyphicon glyphicon-remove' /></div>
    }
    ...
    return (
      <article>
        ...
        {costField}
        <p><span className='glyphicon glyphicon-time' /> {this.props.prepTime + this.props.cookTime}min{cost}</p>
      </article>
    )
  }
};
