...
export default class Shopping extends Component {
    ...
    drop = () => {
        ...
        drag.toIndex =
            (drag.toList === drag.fromList && drag.toIndex < drag.fromIndex) || drag.toList !== drag.fromList
                ? drag.toIndex + 1
                : drag.toIndex
    ...
    add = e => {
        ...
        api.addShoppingListItem(this.datestamp, this.refs.newIngredient.value).then(id => {
            if (!id) return
            var ingredients = this.state.ingredients
    ...
    delete = index => {
        api.deleteShoppingListItem(this.datestamp, this.state.ingredients.fresh[index].id).then(() => {
            var ingredients = this.state.ingredients
    ...
    editPrices = () => {
        ...
        if (this.state.editPrices) {
            ...
            api.saveShoppingList(this.datestamp, ingredients.fresh).then(() => {
                ...
                this.setState({
                    editPrices: !this.state.editPrices,
                    ingredients,
                    offerDefaultPriceChange,
                    cost: cost.cost
                })
                        ...
                        <div className="recipedetail">
                            <h3>
                                ...
                                <span className="float-right">
                                    {this.state.cost &&
                                        this.state.cost.toLocaleString('en-AU', { style: 'currency', currency: 'AUD' })}
                                </span>
                            </h3>
                                        ...
                                        <th colSpan="2" className="ingredient-price">
                                            Price&nbsp;<button
                                                className={`btn btn-xs float-right ${
                                                    this.state.editPrices ? 'btn-success' : 'btn-default'
                                                }`}
                                                onClick={this.editPrices}
                                            >
                                                {this.state.editPrices ? 'save' : 'edit'}
                                            </button>
                                        </th>
                                    </tr>
                                    {this.state.ingredients.fresh &&
                                        this.state.ingredients.fresh.map((r, i) => {
                                            return (
                                                <tr
                                                        ...
                                                        <td className="align-right">
                                                            {this.state.editPrices ? (
                                                                <div className="popside-container">
                                                                    {this.state.offerDefaultPriceChange[
                                                                        r.ingredientId
                                                                    ] && (
                        ...
                        <div className="recipedetail">
                            ...
                            <table className="table" onDrop={this.drop} onDragOver={this.dragOverTable}>
                                <tbody>
                                    ...
                                    {this.state.ingredients.pantry &&
                                        this.state.ingredients.pantry.map((r, i) => {
                                            return (
                                                <tr
